...
namespace V3D_GPU
{
   ...
   ParametricUndistortionFilter::allocate(int w, int h)
   {
      ...
      _floatSrcTex.allocateID();
      _floatSrcTex.reserve(w, h, TextureSpecification("r=32f enableTextureRG"));
      _floatSrcTex.bind(GL_TEXTURE0);
      glTexParameteri(_floatSrcTex.textureTarget(), GL_TEXTURE_MIN_FILTER, GL_LINEAR);
      glTexParameteri(_floatSrcTex.textureTarget(), GL_TEXTURE_MAG_FILTER, GL_LINEAR);

      _destBuffer.allocate(w, h);
      _floatDestBuffer.allocate(w, h);
   ...
   void
   ParametricUndistortionFilter::allocate(int w, int h, std::string cg_File)
   {
      _width = w;
      _height = h;
      ...
      {
         parametricShader = new Cg_FragmentProgram("ParametricUndistortionFilter::parametricShader");
		 parametricShader->setProgramFromFile(cg_File.c_str());
#if 0
         char const * cgArgs[] = { "-bestprecision", "-nofastmath", "-nofastprecision", 0 };
         parametricShader->compile(cgArgs);
#else
   ...
   void
   ParametricUndistortionFilter::undistort(unsigned int texId)
   {
      ...
      parametricShader->parameter("f", float(_fx), float(_fy), float(1.0f/_fx), float(1.0f/_fy));
      parametricShader->parameter("k", float(_k1), float(_k2), float(_k3));
      parametricShader->parameter("p", float(_p1), float(_p2));
      parametricShader->parameter("center", float(_center[0]), float(_center[1]));
      parametricShader->parameter("wh", float(_width), float(_height), float(1.0f/_width), float(1.0f/_height));
      ...
      glVertex2f(0, 0);
      glMultiTexCoord2f(GL_TEXTURE0_ARB, float(2*_width), 0);
      glVertex2f(2, 0);
      glMultiTexCoord2f(GL_TEXTURE0_ARB, 0, float(2*_height));
      glVertex2f(0, 2);
      glEnd();
      parametricShader->disable();

      glDisable(GL_TEXTURE_2D);
   } // end ParametricUndistortionFilter::undistort()
   ...
   void ParametricUndistortionFilter::undistortColorImage(unsigned char const * image) //claudia: without glReadPixels
   {
	   _srcTex.overwriteWith(image, 3);
   ...
   void
   ParametricUndistortionFilter::undistortFloatImage(float const * image)
   {
      _floatSrcTex.overwriteWith(image, 1);

      _floatDestBuffer.activate();
      setupNormalizedProjection();
      ...
      _floatSrcTex.enable(GL_TEXTURE0);
      ...
      _floatSrcTex.disable(GL_TEXTURE0);
   } // end undistortFloatImage()
...
//----------------------------------------------------------------------
   ...
   void Lookup1D_UndistortionFilter::undistortColorImage(unsigned char const * image, unsigned char * result)
   {
      ...
      radialLUTShader->parameter("wh", float(_width), float(_height), float(1.0f/_width), float(1.0f/_height));
      ...
      glVertex2f(0, 0);
	  applyT(_T_rect,2.0f*_width,0.0f,xh,yh,zh);
      glMultiTexCoord3f(GL_TEXTURE0_ARB,xh, yh,zh);
      glVertex2f(2, 0);
	  applyT(_T_rect,0.0f,2.0f*_height,xh,yh,zh);
      glMultiTexCoord3f(GL_TEXTURE0_ARB, xh,yh,zh);
      glVertex2f(0, 2);
      glEnd();
   ...
   Lookup1D_UndistortionFilter::undistortIntensityImage(unsigned char const * image, unsigned char * result)
   {
      ...
      glMultiTexCoord2f(GL_TEXTURE0_ARB, 0, 0);
      glVertex2f(0, 0);
      glMultiTexCoord2f(GL_TEXTURE0_ARB, 2.0f*_width, 0);
      glVertex2f(2, 0);
      glMultiTexCoord2f(GL_TEXTURE0_ARB, 0, 2.0f*_height);
      glVertex2f(0, 2);
      glEnd();
      radialLUTShader->disable();
