...
namespace V3D_GPU
{
   ...
   struct ParametricUndistortionFilter
   {
         ParametricUndistortionFilter()
            : _width(0), _height(0),
              _destBuffer("rgb=8", "ParametricUndistortionFilter::_destBuffer"),
              _floatDestBuffer("r=32f enableTextureRG", "ParametricUndistortionFilter::_floatDestBuffer")
         { }
         ...
         void setDistortionParameters(double const fx, double const fy, double const k1, double const k2, double const k3,
		 ...
		 void allocate(int w, int h, std::string cg_File);

         void deallocate();

         void undistort(unsigned int texId);
         void undistortColorImage(unsigned char const * image);
         ...
         void undistortFloatImage(float const * image); // Renders to the float result buffer

         RTT_Buffer& getResult()      { return _destBuffer; }
         RTT_Buffer& getFloatResult() { return _floatDestBuffer; }
      ...
      protected:
         ...
         ImageTexture2D _srcTex, _floatSrcTex;
         RTT_Buffer  _destBuffer, _floatDestBuffer;
