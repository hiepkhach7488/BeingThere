...
namespace V3D_GPU
{
   ...
   struct ImageTexture2D
   {
         ...
         void readback(uchar * pixels, int nChannels);
         void readback(uchar * redPixels, uchar * greenPixels, uchar * bluePixels);
         void readback(uchar * redPixels, uchar * greenPixels, uchar * bluePixels, uchar * alphaPixels);

         void readback(float * pixels, int nChannels);
         void readback(float * redPixels, float * greenPixels, float * bluePixels);
         void readback(float * redPixels, float * greenPixels, float * bluePixels, float * alphaPixels);
		 ...
		 std::string const& getName() const { return _texName; }
...
# endif
   ...
   struct RTT_Buffer
   {
		 ...
		 std::string const& getTexSpec() const { return _texSpec; }
		 std::string const& getTexName() const { return _tex.getName(); }
...
//----------------------------------------------------------------------
   ...
   struct VertexProgramBase : public ProgramBase
   {
         VertexProgramBase(char const * shaderName)
            : ProgramBase(shaderName)
         { }

         virtual ~VertexProgramBase() { }

         virtual void attribute(char const * name, float x) = 0;
         virtual void attribute(char const * name, float x, float y) = 0;
         virtual void attribute(char const * name, float x, float y, float z) = 0;
         virtual void attribute(char const * name, float x, float y, float z, float w) = 0;
   };
...
#  if defined(V3DLIB_GPGPU_ENABLE_CG)
   ...
   struct Cg_VertexProgram : public VertexProgramBase, public Cg_ProgramBase
   {
         Cg_VertexProgram(char const * shaderName)
            : VertexProgramBase(shaderName), Cg_ProgramBase(),
              _attributeArrayNames()
         { }

         virtual ~Cg_VertexProgram() { }
         ...
         virtual void parameter(char const * name, float x);
         virtual void parameter(char const * name, float x, float y);
         virtual void parameter(char const * name, float x, float y, float z);
         virtual void parameter(char const * name, float x, float y, float z, float w);
         virtual void parameter(char const * name, int len, float const * array);
         virtual void matrixParameterR(char const * param, int rows, int cols, double const * values);
         virtual void matrixParameterC(char const * param, int rows, int cols, double const * values);

         virtual unsigned getTexUnit(char const * param);

         virtual void attribute(char const * name, float x);
         virtual void attribute(char const * name, float x, float y);
         virtual void attribute(char const * name, float x, float y, float z);
         virtual void attribute(char const * name, float x, float y, float z, float w);

         virtual void attributeArray(char const * name, unsigned nComponents, float const * array);

      protected:
         std::vector<std::string> _attributeArrayNames;
   };
...
#  if defined(V3DLIB_GPGPU_ENABLE_CG)
   ...
   void transformImageForPlaneSweepPacked(V3D::CameraMatrix const& srcCam, ImageTexture2D& src, V3D::Vector4f const& depths, V3D::CameraMatrix const& targetCam,
                                          RTT_Buffer& dst, V3D::Vector4f defaultColor = V3D::Vector4f(0, 0, 0, 0));
...
//----------------------------------------------------------------------
   ...
   inline void
   allocateTexturePyramid(int const nLevels, int const w0, int const h0, char const * texSpec, char const * name,
                          std::vector<ImageTexture2D *>& pyr)
   {
      pyr.resize(nLevels);
      for (int level = 0; level < nLevels; ++level)
      {
         int const w = w0 / (1 << level);
         int const h = h0 / (1 << level);

         pyr[level] = new ImageTexture2D(name);
         pyr[level]->allocateID();
         pyr[level]->reserve(w, h, texSpec);
      }
   } // end allocateTexturePyramid()

   inline void
   deallocateTexturePyramid(std::vector<ImageTexture2D *>& pyr)
   {
      int const nLevels = (int) pyr.size();
      for (int level = 0; level < nLevels; ++level)
      {
         pyr[level]->deallocateID();
         delete pyr[level];
      }
      pyr.clear();
   } // end deallocateTexturePyramid()

   inline void
   allocateRTTPyramid(int const nLevels, int const w0, int const h0, char const * texSpec, char const * name,
                      std::vector<RTT_Buffer *>& pyr)
   {
      pyr.resize(nLevels);
      for (int level = 0; level < nLevels; ++level)
      {
         ...
         pyr[level] = new RTT_Buffer(texSpec, name);
         pyr[level]->allocate(w, h);
      }
   } // end allocateRTTPyramid()

   inline void
   deallocateRTTPyramid(std::vector<RTT_Buffer *>& pyr)
   {
      int const nLevels = (int) pyr.size();
      for (int level = 0; level < nLevels; ++level)
      {
         pyr[level]->deallocate();
         delete pyr[level];
      }
      pyr.clear();
   } // end deallocateRTTPyramid()
