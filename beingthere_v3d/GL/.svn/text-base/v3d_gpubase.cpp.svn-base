...
namespace
{
   ...
   template <typename T>
   inline void
   deinterleavePixels(int const w, int const h, T const * pixels, T * red, T * green, T * blue)
   {
      for (int p = 0; p < w*h; ++p)
      {
         red[p]   = pixels[3*p+0];
         green[p] = pixels[3*p+1];
         blue[p]  = pixels[3*p+2];
      }
   } // end interleavePixels()
   ...
   template <typename T>
   inline void
   deinterleavePixels(int const w, int const h, T const * pixels, T * red, T * green, T * blue, T * alpha)
   {
      for (int p = 0; p < w*h; ++p)
      {
         red[p]   = pixels[4*p+0];
         green[p] = pixels[4*p+1];
         blue[p]  = pixels[4*p+2];
         alpha[p] = pixels[4*p+3];
      }
   } // end interleavePixels()
...
//----------------------------------------------------------------------
   ...
   void
   ImageTexture2D::readback(uchar * pixels, int nChannels)
   {
      unsigned const format = extPixelFormat[nChannels];
      unsigned const type = GL_UNSIGNED_BYTE;

      glBindTexture(_textureTarget, _textureID);
      glGetTexImage(_textureTarget, 0, format, type, pixels);
      checkGLErrorsHere1(_texName);
   }

   void
   ImageTexture2D::readback(uchar * redPixels, uchar * greenPixels, uchar * bluePixels)
   {
      unsigned const format = extPixelFormat[3];
      unsigned const type = GL_UNSIGNED_BYTE;
      ...
      glBindTexture(_textureTarget, _textureID);
      glGetTexImage(_textureTarget, 0, format, type, pixels);
      deinterleavePixels(_width, _height, pixels, redPixels, greenPixels, bluePixels);
   ...
   void
   ImageTexture2D::readback(uchar * redPixels, uchar * greenPixels, uchar * bluePixels, uchar * alphaPixels)
   {
      unsigned const format = extPixelFormat[4];
      unsigned const type = GL_UNSIGNED_BYTE;
      ...
      glBindTexture(_textureTarget, _textureID);
      glGetTexImage(_textureTarget, 0, format, type, pixels);
      deinterleavePixels(_width, _height, pixels, redPixels, greenPixels, bluePixels, alphaPixels);
   ...
   void
   ImageTexture2D::readback(float * pixels, int nChannels)
   {
      unsigned const format = extPixelFormat[nChannels];
      unsigned const type = GL_FLOAT;
   ...
   void
   ImageTexture2D::readback(float * redPixels, float * greenPixels, float * bluePixels)
   {
      unsigned const format = extPixelFormat[3];
      unsigned const type = GL_FLOAT;
   ...
   void
   ImageTexture2D::readback(float * redPixels, float * greenPixels, float * bluePixels, float * alphaPixels)
   {
      unsigned const format = extPixelFormat[4];
      unsigned const type = GL_FLOAT;
...
namespace V3D_GPU
{
   ...
   ProgramBase::setProgramFromFile(char const * fileName)
   {
	  ...
	  if(true)
      {
         ...
         strncpy_s(completeName, fileName, 1024);
      }
      else
      {
...
//======================================================================
   ...
   void
   Cg_VertexProgram::setProgram(char const * source)
   {
      _source = source;
      if (_program != 0) cgDestroyProgram(_program);
      _program = 0;
   ...
   void
   Cg_VertexProgram::compile(char const * * compilerArgs, char const *entry)
   {
      if (_program == 0)
      {
         _program = cgCreateProgram(_context, CG_SOURCE, _source.c_str(),
                                    vertexProfile, entry, compilerArgs);
         if (_program == 0)
         {
            cerr << "Program: " << _shaderName << endl;
            cerr << "Cg compiler report:" << endl << cgGetLastListing(_context) << endl;
   ...
   void
   Cg_VertexProgram::compile(std::vector<std::string> const& compilerArgs, char const *entry)
   {
      size_t const nArgs = compilerArgs.size();
      char const * * args = new char const* [nArgs + 1];
      for (size_t i = 0; i < nArgs; ++i)
   ...
   void
   Cg_VertexProgram::enable()
   {
      cgGLEnableProfile(vertexProfile);
      cgGLLoadProgram(_program);
      cgGLBindProgram(_program);
      for (unsigned i = 0; i < _attributeArrayNames.size(); ++i)
         cgGLEnableClientState(cgGetNamedParameter(_program, _attributeArrayNames[i].c_str()));
   }

   void 
   Cg_VertexProgram::disable()
   {
      cgGLDisableProfile(vertexProfile);
      for (unsigned i = 0; i < _attributeArrayNames.size(); ++i)
         cgGLDisableClientState(cgGetNamedParameter(_program, _attributeArrayNames[i].c_str()));
   }


   void
   Cg_VertexProgram::parameter(char const * name, float x)
   {
      cgSetParameter1f(cgGetNamedParameter(_program, name), x);
   }

   void
   Cg_VertexProgram::parameter(char const * name, float x, float y)
   {
      cgSetParameter2f(cgGetNamedParameter(_program, name), x, y);
   }

   void
   Cg_VertexProgram::parameter(char const * name, float x, float y, float z)
   {
      cgSetParameter3f(cgGetNamedParameter(_program, name), x, y, z);
   }

   void
   Cg_VertexProgram::parameter(char const * name, float x, float y, float z, float w)
   {
      cgSetParameter4f(cgGetNamedParameter(_program, name), x, y, z, w);
   }

   void
   Cg_VertexProgram::parameter(char const * name, int len, float const * array)
   {
      cgGLSetParameterArray1f(cgGetNamedParameter(_program, name), 0, len, array);
   }

   void
   Cg_VertexProgram::matrixParameterR(char const * param, int rows, int cols, double const * values)
   {
      if (rows > 4 || cols > 4)
      {
         raiseGLErrorHere2("Matrix parameter should be <= 4x4.", _shaderName.c_str());
   ...
   void
   Cg_VertexProgram::matrixParameterC(char const * param, int rows, int cols, double const * values)
   {
      if (rows > 4 || cols > 4)
      {
         raiseGLErrorHere2("Matrix parameter should be <= 4x4.", _shaderName.c_str());
   ...
   unsigned
   Cg_VertexProgram::getTexUnit(char const * param)
   {
      return cgGLGetTextureEnum(cgGetNamedParameter(_program, param));
   }

   void
   Cg_VertexProgram::attribute(char const * name, float x)
   {
      cgSetParameter1d(cgGetNamedParameter(_program, name), x);
   }

   void
   Cg_VertexProgram::attribute(char const * name, float x, float y)
   {
      cgSetParameter2d(cgGetNamedParameter(_program, name), x, y);
   }

   void
   Cg_VertexProgram::attribute(char const * name, float x, float y, float z)
   {
      cgSetParameter3d(cgGetNamedParameter(_program, name), x, y, z);
   }

   void
   Cg_VertexProgram::attribute(char const * name, float x, float y, float z, float w)
   {
      cgSetParameter4d(cgGetNamedParameter(_program, name), x, y, z, w);
   }

   void
   Cg_VertexProgram::attributeArray(char const * name, unsigned nComponents, float const * array)
   {
      cgGLSetParameterPointer(cgGetNamedParameter(_program, name),
                              nComponents, GL_FLOAT, 0, array);
      _attributeArrayNames.push_back(string(name));
   }
...
// ----------------------------------------------------------------------
   ...
   void
   transformImageForPlaneSweepPacked(CameraMatrix const& srcCam, ImageTexture2D& src, Vector4f const& depths, CameraMatrix const& targetCam,
                                     RTT_Buffer& dst, Vector4f defaultColor)
   {
      static Cg_FragmentProgram * shader = 0;

      if (shader == 0)
      {
         shader = new Cg_FragmentProgram("transformImageForPlaneSweepPacked::shader");
         shader->setProgramFromFile("warp_homography_packed.cg");
         shader->compile();
         checkGLErrorsHere0();
      } // end if

      Matrix3x4d const RT = getRelativeOrientation(targetCam.getOrientation(), srcCam.getOrientation()); // relative orientation between the cameras

      CameraMatrix cam0;
      CameraMatrix cam1;
      cam1.setOrientation(RT);

      Matrix3x3d const K0 = targetCam.getIntrinsic();
      Matrix3x3d const K1 = srcCam.getIntrinsic();
      Matrix3x3d const invK0 = invertedMatrix(K0);

      Matrix3x3d S0, S1;
      makeIdentityMatrix(S0);
      makeIdentityMatrix(S1);
      ...
      S1[0][0] = 1.0f / src.width();
      S1[1][1] = 1.0f / src.height();
      S1[0][2] = 0.5f / src.width();
      S1[1][2] = 0.5f / src.height();

      Matrix3x3d const Left = S1 * K1;
      Matrix3x3d const Right = invK0 * S0;

      Vector3d   const n(0.0f, 0.0f, -1.0f);
      Matrix3x3d const R = cam1.getRotation();
      Vector3d         T = cam1.getTranslation();
      //normalizeVector(T);

      // cout << "Left = "; displayMatrix(Left);
      // cout << "Right = "; displayMatrix(Right);
      // cout << "R = "; displayMatrix(R);
      // cout << "T = "; displayVector(T);

      Matrix3x3d I, tn, H0, H1, H2, H3;
      makeIdentityMatrix(I);
      makeOuterProductMatrix(T, n, tn);

      H0 = Left * (R - (1.0 / depths[0]) * tn) * Right;
      H1 = Left * (R - (1.0 / depths[1]) * tn) * Right;
      H2 = Left * (R - (1.0 / depths[2]) * tn) * Right;
      H3 = Left * (R - (1.0 / depths[3]) * tn) * Right;

      // cout << "H0_norm = "; displayMatrix(R - (1.0 / depths[0]) * tn);
      // cout << "H0 = "; displayMatrix(H0);

      dst.activate();
      setupNormalizedProjection();

      Vector3d const p00(0, 0, 1);
      Vector3d const p20(2, 0, 1);
      Vector3d const p02(0, 2, 1);

      Vector3d H0_p00 = H0 * p00;
      Vector3d H0_p20 = H0 * p20;
      Vector3d H0_p02 = H0 * p02;

      Vector3d H1_p00 = H1 * p00;
      Vector3d H1_p20 = H1 * p20;
      Vector3d H1_p02 = H1 * p02;

      Vector3d H2_p00 = H2 * p00;
      Vector3d H2_p20 = H2 * p20;
      Vector3d H2_p02 = H2 * p02;

      Vector3d H3_p00 = H3 * p00;
      Vector3d H3_p20 = H3 * p20;
      Vector3d H3_p02 = H3 * p02;

      src.enable(GL_TEXTURE0_ARB);

      shader->enable();
      glMultiTexCoord3fv(GL_TEXTURE4_ARB, &defaultColor[0]);
      glBegin(GL_TRIANGLES);
      glMultiTexCoord3dv(GL_TEXTURE0_ARB, &H0_p00[0]);
      glMultiTexCoord3dv(GL_TEXTURE1_ARB, &H1_p00[0]);
      glMultiTexCoord3dv(GL_TEXTURE2_ARB, &H2_p00[0]);
      glMultiTexCoord3dv(GL_TEXTURE3_ARB, &H3_p00[0]);
      glVertex2f(0, 0);
      glMultiTexCoord3dv(GL_TEXTURE0_ARB, &H0_p20[0]);
      glMultiTexCoord3dv(GL_TEXTURE1_ARB, &H1_p20[0]);
      glMultiTexCoord3dv(GL_TEXTURE2_ARB, &H2_p20[0]);
      glMultiTexCoord3dv(GL_TEXTURE3_ARB, &H3_p20[0]);
      glVertex2f(2, 0);
      glMultiTexCoord3dv(GL_TEXTURE0_ARB, &H0_p02[0]);
      glMultiTexCoord3dv(GL_TEXTURE1_ARB, &H1_p02[0]);
      glMultiTexCoord3dv(GL_TEXTURE2_ARB, &H2_p02[0]);
      glMultiTexCoord3dv(GL_TEXTURE3_ARB, &H3_p02[0]);
      glVertex2f(0, 2);
      glEnd();
      shader->disable();

      src.disable(GL_TEXTURE0_ARB);
   } // end transformImageForPlaneSweepPacked()
