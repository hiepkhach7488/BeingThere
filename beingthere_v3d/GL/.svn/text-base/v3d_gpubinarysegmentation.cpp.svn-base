...
namespace V3D_GPU
{
   ...
   BinarySegmentationUzawa::iterate(unsigned int costTexId, unsigned int edgeWeightTexId, int nIterations)
   {
      ...
      if (uShader == 0)
      {
		 ...
		 uShader->setProgramFromFile((SHADER_ROOT_PATH + "binary_segmentation_uzawa_update_u.cg").c_str());
         uShader->compile();
         checkGLErrorsHere0();
      }
      ...
      if (pShader == 0)
      {
		 ...
		 pShader->setProgramFromFile((SHADER_ROOT_PATH +"binary_segmentation_uzawa_update_p.cg").c_str());
         pShader->compile();
         checkGLErrorsHere0();
      }
