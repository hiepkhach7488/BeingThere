...
GPU_DepthInpainting::iterate(unsigned int origTexId, unsigned int edgeTexId)
{
	...
	if (shader_u == 0)
	{
		...
		shader_u->setProgramFromFile("D:/Trunk/bibleProj/beingthere/examples/opengl_cg_segmentation/shader/depth_inpainting_relaxed_update_u.cg");
		shader_u->compile();
		checkGLErrorsHere0();
	}
	if (shader_p == 0)
	{
		...
		shader_p->setProgramFromFile("D:/Trunk/bibleProj/beingthere/examples/opengl_cg_segmentation/shader/depth_inpainting_relaxed_update_p.cg");
		shader_p->compile();
		checkGLErrorsHere0();
	}
