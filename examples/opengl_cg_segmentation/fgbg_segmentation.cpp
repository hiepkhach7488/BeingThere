...
void FGBG_Segmentation::run(unsigned int const bgColorTexId, unsigned int const bgDepthTexId, unsigned int const curColorTexId, unsigned int const curDepthTexId)
{
...
#elif 1
	{
		...
		shader2->enable();
		//glMultiTexCoord1f(GL_TEXTURE4, 0.2); old
		glMultiTexCoord1f(GL_TEXTURE4, 120.0);
		V3D_GPU::renderNormalizedQuad();
		shader2->disable();
...
#endif
	//renderSegmentationCost(_lambda_color, _color_bias, _lambda_depth, _depth_bias, bgColorTexId, bgDepthTexId, curColorTexId, curDepthTexId, _costBuf);
...
void 
FGBG_Segmentation::renderEdgeWeights(float const alpha, unsigned int texId, V3D_GPU::RTT_Buffer& dst)
{
	static V3D_GPU::Cg_FragmentProgram * shader = 0;
	if (shader == 0)
	{
		shader = new V3D_GPU::Cg_FragmentProgram("renderEdgeWeights::shader");
	...
	V3D_GPU::renderNormalizedQuad(V3D_GPU::GPU_SAMPLE_NEIGHBORS, ds, dt);
	shader->disable();
