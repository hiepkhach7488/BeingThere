...
void main(void){	
	...
	if(d0 != 0.0 && d1 !=0.0 && d2 !=0.0 && abs(d0-d1) <= maxOffset && abs(d0-d2) <= maxOffset && 
		abs(d1-d2) <= maxOffset && d0*DEPTH_TO_CM<500) {
			...
			nDotL = dot(calcNormal(gl_in[0].gl_Position.xy, vec3(pos)), normalize(vec3(-pos)));
			EmitVertex();
			...
			gl_Position = MVP * pos;
			distanceToCam = distance(vec3(pos), vec3(0,0,0));
			nDotL = dot(calcNormal(gl_in[1].gl_Position.xy, vec3(pos)), normalize(vec3(-pos)));
			EmitVertex();
			...
			gl_Position = MVP * pos;
			distanceToCam = distance(vec3(pos), vec3(0,0,0));
			nDotL = dot(calcNormal(gl_in[2].gl_Position.xy, vec3(pos)), normalize(vec3(-pos)));
			EmitVertex();
