...
namespace{
	...
	std::string const SHADER_ROOT_PATH = "D:/Trunk/bibleProj/beingthere/examples/opengl_geometry_shader/";
	std::string const VERT_SHADER_SOURCE(SHADER_ROOT_PATH + "shader/geometry.vert");
	std::string const GEOM_SHADER_SOURCE(SHADER_ROOT_PATH + "shader/geometry.geom");
	std::string const FRAG_SHADER_SOURCE(SHADER_ROOT_PATH + "shader/geometry.frag");
}
...
void updateMVPOrtho(glm::mat4& MVP){
	glm::mat4 Projection = glm::ortho(0.f, 640.f, 0.f, 480.f, -1.f, 1.f);
...
void updateMVPPersp(glm::mat4& MVP){
	glm::mat4 Projection = glm::perspective(45.0f, 640.0f / 480.0f, 1.f, 5000.f);
	glm::mat4 View = glm::translate(glm::mat4(1.f), glm::vec3(0.f, 0.f, -500.f));
	glm::mat4 Model = glm::mat4(1.0f);
...
void createShader(){
	...
	textureShader->compileShaderFromFile(VERT_SHADER_SOURCE.c_str(), GLSLShader::VERTEX);   cout << textureShader->log() << endl;
	textureShader->compileShaderFromFile(GEOM_SHADER_SOURCE.c_str(), GLSLShader::GEOMETRY); cout << textureShader->log() << endl;
	textureShader->compileShaderFromFile(FRAG_SHADER_SOURCE.c_str(), GLSLShader::FRAGMENT); cout << textureShader->log() << endl;
	textureShader->link(); cout << textureShader->log() << endl;
	textureShader->validate(); cout << textureShader->log() << endl;
