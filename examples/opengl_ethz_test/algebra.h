...
#define ALGEBRA_ERROR(E) { assert(false); }
  ...
  const float M2_PI = float(M_PI*2.0);				// PI*2
  ...
  inline vec2& vec2::operator /= ( const float d )
  { float d_inv = 1.f/d; n[VX] *= d_inv; n[VY] *= d_inv; return *this; }
  ...
  inline vec2 operator / (const vec2& a, const float d)
  { float d_inv = 1.f/d; return vec2(a.n[VX]*d_inv, a.n[VY]*d_inv); }
  ...
  inline vec3& vec3::operator /= ( const float d )
  { float d_inv = 1.f/d; n[VX] *= d_inv; n[VY] *= d_inv; n[VZ] *= d_inv;
    return *this; }
...
#undef ROWCOL // (i)
  ...
  inline vec3 operator / (const vec3& a, const float d)
  { float d_inv = 1.f/d; return vec3(a.n[VX]*d_inv, a.n[VY]*d_inv,
				    a.n[VZ]*d_inv); }
...
#undef ROWCOL // (i)
  ...
  inline vec4 operator / (const vec4& a, const float d)
  { float d_inv = 1.f/d; return vec4(a.n[VX]*d_inv, a.n[VY]*d_inv, a.n[VZ]*d_inv,
				    a.n[VW]*d_inv); }
  ...
  inline float* mat3::ptr()
  {
	  return v[0].ptr();
  }

  inline const float* mat3::ptr() const
  {
	  return v[0].ptr();
  }
